...
dockprom [origin](https://github.com/stefanprodan/dockprom.git)
========
...
Clone this repository on your Docker host, cd into tuanlinhchi-monitorserver directory and run compose up:

```bash
git clone https://github.com/nghiemtientuan/tuanlinhchi-monitorserver.git
cd tuanlinhchi-monitorserver
...
Go to `http://<host-ip>:3000` and login with user ***admin*** password ***admin***. You can change the credentials in the compose file or by supplying the `ADMIN_USER` and `ADMIN_PASSWORD` environment variables on compose up. The config file can be added directly in grafana part like this
